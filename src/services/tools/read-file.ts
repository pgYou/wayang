...
export function readFileTool(deps: { cwd?: string } = {}) {
  ...
  return defineTool({
    description: `Read file content (with line numbers). Returns at most ${DEFAULT_MAX_LINES} lines by default. Use offset (1-based start line) and limit to read specific sections of large files. Path must be within the workspace.`,
    parameters: z.object({
